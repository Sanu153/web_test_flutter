...
//    onConnection();
  ...
  void unSubscribeChanel() {
    // Make all unsubscribeToChannel
    _actionCable.unsubscribeToChannel(BUY_SELL_COUNT_CHANNEL);
    _actionCable.unsubscribeToChannel(NEGOTIATION_CHANNEL);
    _actionCable.unsubscribeToChannel(GRAPHDATA_CHANNEL);
    _actionCable.unsubscribeToChannel(PORTFOLIO_OPEN_CHANNEL);
    _actionCable.unsubscribeToChannel(PORTFOLIO_CLOSE_CHANNEL);
...
//      //print(e);
  ...
  void buySellCount(int productSpecId, int marketId,
    ...
    _actionCable.subscribeToChannel(BUY_SELL_COUNT_CHANNEL,
//        channelParams: {'product_spec_id': productSpecId,"market_hierarchy_id": marketId},
  ...
  void chatNegotiation(int requestResponder,
      {Function onData, Function onSubscribed}) {
    _actionCable.subscribeToChannel(NEGOTIATION_CHANNEL,
        channelParams: {'request_responder_id': requestResponder},
        onSubscribed: onSubscribed, onMessage: (Map data) {
      ...
      if (data.containsKey("negotiation_chat_message")) {
        ...
        final ApproveRejectMessage _sts = ApproveRejectMessage(
            message:
                "${data['response_message']} by ${data['user']['first_name']} ${data['user']['last_name'] ?? ''} ${_dataManager.authUser.user.userId == data['user']['id'] ? '(you)' : ''}",
...
//    //print("Connection: ${_actionCable.}")
    _actionCable.subscribeToChannel(GRAPHDATA_CHANNEL,
  ...
  void portfolioOPenChannel({Function onData, Function onSubscribed}) {
    ...
    _actionCable.subscribeToChannel(PORTFOLIO_OPEN_CHANNEL,
  ...
  void portfolioClosedChannel({Function onData, Function onSubscribed}) {
    _actionCable.subscribeToChannel(PORTFOLIO_CLOSE_CHANNEL,
        channelParams: {"user_id": _dataManager.authUser.user.userId},
        onSubscribed: () {
          ...
          onData(OpenNegotiation.fromJson(data['portfolio_closed']));
        });
  }
